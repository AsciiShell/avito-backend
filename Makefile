IMPORT_PATH ?= github.com/asciishell/avito-backend
BUILD_DIR ?= bin
PKG_DIR = .pkg
GOROOT ?= /usr/local/go
...
.PHONY: ci-deploy
ci-deploy:
	ssh -t root@$$TARGET_HOST 'cd messenger-api && docker-compose stop'
	scp ./docker-compose.yml root@$$TARGET_HOST:messenger-api/docker-compose.yml
	ssh -t root@$$TARGET_HOST 'cd messenger-api && IMAGE_TAG=$(DOCKER_IMAGE_TAG) docker-compose pull && IMAGE_TAG=$(DOCKER_IMAGE_TAG) docker-compose up -d'
